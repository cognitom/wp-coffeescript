...
class WpRiottag {
  ...
  function __construct() {
    ...
    $this->output_directory_path = $upload_directory['basedir'].'/riottag-cache';
    $this->output_directory_url = $upload_directory['baseurl'].'/riottag-cache';

    $this->executable = apply_filters('wprt_executable', $this->executable);
    $this->output_directory_path = apply_filters('wprt_output_directory_path', $this->output_directory_path);
    $this->output_directory_url = apply_filters('wprt_output_directory_url', $this->output_directory_url);
    $this->caching_enabled = apply_filters('wprt_caching_enabled', $this->caching_enabled);
  ...
  public function enqueue($handle, $input_path_or_paths, $deps=array(), $ver=false, $in_footer=false) {
    ...
    $this->error('Unable to compile Riot Tag with the handle "'.$handle.'".');
  ...
  public function compile($input_path, $handle) {
    ...
    if ($compilation_necessary) {
      ...
      $command = $escaped_executable.' '.$escaped_input_path.' --stdout 2>&1';
  ...
  public function compile_multiple($input_paths, $handle) {
    ...
    if ($compilation_necessary) {
      ...
      $command = 'echo "'.$escaped_input_paths.'" | '.$escaped_executable.' --stdin -stdout 2>&1';
  ...
  private function error($message, $error_type=E_USER_WARNING) {
    if ($this->throw_errors) {
      trigger_error('WP Riot Tag: '.$message, $error_type);
    }
  }
